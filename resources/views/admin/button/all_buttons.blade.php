...
<head>
    <meta charset="utf-8" />
    <title>All Buttons | Silai Point </title>
    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-header border-bottom border-dashed d-flex align-items-center justify-content-between">
                <h4 class="header-title mb-0">Button List</h4>
            </div>
...
<tr>
    ...
    <!-- CATEGORY -->
    <td>
        {{ $button->category->name ?? 'N/A' }}
    </td>

    <!-- CODE -->
    <td>{{ $button->code ?? 'N/A' }}</td>

    <!-- COLOR -->
    <td>{{ $button->color ?? 'N/A' }}</td>

    <!-- SIZE -->
    <td>{{ $button->size ?? 'N/A' }}</td>

    <!-- PRICE -->
    <td>Rs. {{ number_format($button->price ?? 0) }}</td>

    <!-- STOCK -->
    <td>{{ $button->stock ?? 0 }}</td>
    ...
    <td class="text-center">
        ...
        <a href="{{ route('edit.button', $button->id) }}"
           class="text-primary fs-20 p-1">
            <i class="ti ti-pencil"></i>
        </a>

        <form action="{{ route('delete.button', $button->id) }}"
              method="POST"
              class="d-inline">
