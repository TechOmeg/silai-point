...
@if(session('cart') && count(session('cart')) > 0)
    ...
    $meter = $details['meter'] ?? 1;
    $total = $details['total_price'] ?? ($price * $meter);
...
@endphp
    ...
    if(($details['type'] ?? '') == 'button'){
        ...
        $imagePath = asset('uploads/buttons/' . $details['image']);

    }else{

        $imagePath = asset('uploads/fabrics/' . $details['image']);
...
@endphp
            ...
            <td class="cart__table--body__list">
                <span class="cart__price">
                    Rs {{ number_format($details['price_per_meter'] ?? 0) }}
                </span>
            </td>
...
<td class="cart__table--body__list">
    <span class="cart__price end">
        Rs {{ number_format($details['total_price'] ?? 0) }}
    </span>
</td>
        </tr>
...
@endif
    ...
    <h3 class="coupon__code--title">
        Fabric & Buttons Selection
    </h3>

    <p class="coupon__code--desc">
        Aap apna pasandida fabric aur buttons cart me add kar chuke hain. 
        Ab agay measurement form fill karein takay aapka custom kameez shalwar perfectly stitch ho sake.
    </p>
    ...
    <div class="coupon__code--field">
        ...
        <div style="
            ...
            color:#444;
        ">
            Side me diye gaye demo ko dekh kar apni measurements enter karein.
            Har measurement sahi dena zaroori hai takay fitting perfect aaye.
        </div>
    ...
    <p class="cart__note--desc">
        Yahan apni special stitching instructions likhein.
    </p>

    <textarea
        class="cart__note--textarea border-radius-5"
        placeholder="Jaise: loose fitting, slim fit, extra pocket, cuff style etc..."
    ></textarea>
        ...
        Rs {{ number_format($subtotal) }}
                                    ...
                                    <div class="cart__summary--footer">
                                        ...
                                        <ul class="d-flex justify-content-between">
                                            ...
                                            <li><a class="cart__summary--footer__btn primary__btn checkout" href="{{route('order.overview')}}">Order Now</a></li>
                                        </ul>
                                    </div>
                                </div> 
                            </div>
